...
async def compute_live_networth(session: Session, user_id: int) -> NetWorthResult:
    ...
    for acc in included:
        try:
            value_base = convert_to_base(acc.balance, acc.currency, base_currency, rates)
...
async def compute_projection_series(session: Session, user_id: int, years: int = 10):
    ...
    Returns: (base_currency, fx_as_of, excluded_accounts, points)
    ...
    # Count excluded accounts based on current FX coverage
    excluded_current = 0
    for acc in state:
        try:
            _ = convert_to_base(acc["balance"], acc["currency"], base_currency, rates)
        except HTTPException:
            excluded_current += 1
    ...
    for m in range(0, months + 1):
        ...
        for acc in state:
            try:
                total += float(convert_to_base(acc["balance"], acc["currency"], base_currency, rates))
            except HTTPException:
                excluded += 1
                continue
        ...
        points.append({
            ...
            "projected_net_worth": round(total, 2),
        })
    ...
    return {
        "base_currency": base_currency,
        "fx_as_of": str(fx_cache.cache_date),
        "excluded_accounts": excluded_current,  # based on current FX coverage not super meaningful for projection; keep for API symmetry
        "years": years,
        "points": points,
    }
