...
export default function PlanIsaStrategyCard({
  ...
  useEffect(() => {
    ...
    track?.('plan_isa_card_viewed', {
      wrapper: guidance.wrapperKey,
      confidence: guidance.confidence,
      status,
    })
  }, [guidance.wrapperKey, guidance.confidence, status, track])
  ...
  const wrapperCards = [
    {
      ...
      body: 'Best default for long-term plan funding where growth matters more than short-term stability.',
    },
    {
      key: 'cash',
      icon: PiggyBank,
      title: 'Cash ISA',
      body: 'Better fit for shorter horizons, near-term use, or when protecting capital matters more than return.',
    },
    {
      key: 'lisa',
      icon: House,
      title: 'Lifetime ISA',
      body: 'Powerful for eligible first-home or later-life saving, but stricter withdrawal rules make fit important.',
    },
  ]
    ...
    <Card className={`${planTheme.sectionCard} p-5 sm:p-6`}>
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="min-w-0">
          ...
          <div className={`mt-2 ${planTheme.body} max-w-[46rem]`}>
            See your remaining ISA room, whether your current pace uses it, and which wrapper best
            fits the plan right now.
          </div>
        </div>
      ...
      <div className="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className={`${planTheme.innerCard} p-4`}>
          ...
          <div className="mt-2 text-lg font-display text-ink dark:text-white">
            {fmtCurrency(rules.lisaAllowance, 'GBP')}
          </div>
        </div>
      ...
      <div className="mt-6 grid grid-cols-1 xl:grid-cols-[1.08fr_0.92fr] gap-6">
        ...
        <div>
          <div className={planTheme.statLabel}>Best wrapper right now</div>

          <div className="mt-3 rounded-2xl border border-accent/18 bg-accent/[.05] dark:bg-accent/[.10] p-4">
          ...
          <div className="mt-3 space-y-3">
            {wrapperCards.map(({ key, icon: Icon, title, body }) => {
              const active = key === wrapperKey
              ...
              return (
                <div
                  ...
                  className={[
                    ...
                    active
                      ? 'border-accent/18 bg-accent/[.04] dark:bg-accent/[.08]'
                      : 'border-black/[.05] dark:border-white/[.06] bg-black/[.02] dark:bg-white/[.04]',
                  ].join(' ')}
                >
                  <div className="flex items-start gap-3">
                    <div className="w-9 h-9 rounded-2xl flex items-center justify-center border border-black/[.06] dark:border-white/[.08] bg-white/75 dark:bg-white/[.05]">
                      <Icon size={16} className="text-ink dark:text-white" />
                    </div>

                    <div className="min-w-0">
                      <div className="text-sm font-semibold text-ink dark:text-white">{title}</div>
                      <div className={`mt-1 ${planTheme.body}`}>{body}</div>
                    </div>
                  </div>
                </div>
              )
          ...
          <div className={`mt-5 pt-5 border-t ${planTheme.divider}`}>
            <div className="text-xs leading-relaxed text-ink-muted dark:text-white/34">
              ...
              standard ISA. Also note that replacing withdrawn ISA money in the same tax year only
              preserves allowance in flexible ISAs.
            </div>
          </div>
        </div>
      </div>
